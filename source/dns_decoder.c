...
static int s_decode_name(struct aws_byte_buf *buf, struct aws_byte_cursor whole_packet_cursor, struct aws_byte_cursor *response_packet_cursor) {

    struct aws_byte_cursor compression_dummy = whole_packet_cursor;
    ...
    while (label_length > 0) {
        ...
        if (label_length > name_cursor->len) {
            return AWS_OP_ERR;
        }
...
static int s_decode_resource_record(struct aws_byte_cursor whole_packet_cursor, struct aws_byte_cursor *response_packet_cursor, struct aws_allocator *allocator, struct aws_dns_resource_record *record, bool is_question) {
...
static int s_decode_resource_record_list(struct aws_byte_cursor whole_packet_cursor, struct aws_byte_cursor *response_packet_cursor, struct aws_allocator *allocator, struct aws_array_list *records, uint16_t record_count, bool is_question) {
...
int aws_dns_decode_response(struct aws_dns_query_result *result, struct aws_allocator *allocator, struct aws_byte_cursor response_packet_cursor) {
    ...
    if (s_decode_resource_record_list(whole_packet_cursor, &response_packet_cursor, allocator, &result->question_records, question_count, true)) {
        return AWS_OP_ERR;
    }

    if (s_decode_resource_record_list(whole_packet_cursor, &response_packet_cursor, allocator, &result->answer_records, answer_count, false)) {
        return AWS_OP_ERR;
    }

    if (s_decode_resource_record_list(whole_packet_cursor, &response_packet_cursor, allocator, &result->authority_records, authority_count, false)) {
        return AWS_OP_ERR;
    }

    if (s_decode_resource_record_list(whole_packet_cursor, &response_packet_cursor, allocator, &result->additional_records, additional_count, false)) {
        return AWS_OP_ERR;
    }
